<section class="space-y-6">
    ...
    <x-danger-button x-data=""
        x-on:click.prevent="$dispatch('open-modal', 'confirm-delete-service')">
        {{ __('Delete service') }}
    </x-danger-button>

    <!-- Modal -->
    <x-modal name="confirm-delete-service" focusable class="fixed inset-0 z-[9999]">
        <div class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-6">
            <form method="post" action="{{ route('service.destroy', $service) }}" 
                ...
                <h2 class="text-lg font-medium text-gray-900 dark:text-gray-100">
                    {{ __('Are you sure you want to delete service?') }}
                </h2>
